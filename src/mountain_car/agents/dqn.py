...
class DQNAgent:
    ...
    def __init__(
        ...
        *,
        ...
        self._last_exploration_action: int | None = None
    ...
    def select_action(self, state: np.ndarray, *, deterministic: bool = False) -> int:
        ...
        if not deterministic and self._exploration_steps_left > 0:
            # Mantener el empuje permite que el coche gane impulso.
            self._exploration_steps_left -= 1
            return self._last_exploration_action  # type: ignore[return-value]

        if not deterministic and random.random() < self.epsilon:
            self._last_exploration_action = random.randrange(self.action_dim)
            self._exploration_steps_left = max(0, self.exploration_duration - 1)
            return self._last_exploration_action

        self._last_exploration_action = None
        self._exploration_steps_left = 0
        with torch.no_grad():
    ...
    def _learn(self) -> float:
        ...
        # Se elige el valor de la acción que realmente se tomó.
        current_q = self.q_net(states_t).gather(1, actions_t)

        # La red objetivo calcula el siguiente valor sin recibir gradientes.
        with torch.no_grad():
    ...
    def train(self, total_episodes: int = 500, log_interval: int = 10) -> list[float]:
        ...
        for episode in range(1, total_episodes + 1):
            obs, _ = env.reset()
            # La acción exploratoria pegajosa no debe pasar de un episodio a otro.
            self._last_exploration_action = None
            self._exploration_steps_left = 0
            total_reward = 0.0
            done = False
    ...
    def load(cls, path: Path) -> Self:
        ...
        agent = cls(
            ...
            **{
                k: data.get(k, 20 if k == "exploration_duration" else None)
